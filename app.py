...
def make_comparison_image(image, size=(600, 600)):
    """Place an image on an equal-sized white canvas."""
    return ImageOps.pad(
        image.convert("RGB"),
        size,
...
with ai_tab:
    ...
    if background_history:
        ...
        for index, history_background in enumerate(
            ...
            with history_columns[index % 3]:
                background_thumbnail = history_background.copy()
        ...
        # Display the final photo alone above the controls
        with final_photo_placeholder.container():
            final_display_column, empty_column = st.columns(
                [3, 1]
            )

            with final_display_column:
                st.image(
                    final_product_image,
                    width="stretch",
                )
        ...
        # Display equal-sized comparison after the controls
        st.subheader("Before & Final Comparison")

        before_image = make_comparison_image(
            product_image,
            size=(600, 600),
        )

        after_image = make_comparison_image(
            final_product_image,
            size=(600, 600),
        )
